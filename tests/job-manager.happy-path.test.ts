...
describe('JobManager (happy path)', () => {
  it('runs a role session, verifies, commits, and writes status', async () => {
    const { dir: repoRoot } = await createTempGitRepo();
    // Mirror real repo ignore behavior so evidence isn't treated as untracked changes.
    ...
    expect(status).toContain('"current_role": null');
  });
});
